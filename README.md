...
- 歌词翻译显示：网易云、QQ 音乐提供翻译轨道时自动挂载，卡片歌词页逐行显示，设置中可开关。
- 顶栏副行（Lyricify 双行模式）：优先显示当前行翻译，无翻译时显示下一句预览，可在设置中开关。
- 灵动词岛式自适应宽度（可选）：顶栏歌词条随当前行内容伸缩，上限为固定宽度设置；关闭时保持固定宽度。
- 外观自定义：卡拉OK填充色可跟随主题强调色或使用自定义十六进制颜色，顶栏歌词不透明度与字体家族可调。
- 歌词纠错：卡片显示当前歌词来源，提供"重新匹配"按钮列出各在线源候选，手动选定后按曲目记忆，可一键恢复自动匹配。
- 歌词偏移校正：展开卡片歌词页后可按 ±0.5s 微调歌词时序，按歌曲记忆，设置中可一键清理偏移记录、手动选择记录与封面缓存。
